...
void BOARD::Init(char Board[32], int Piece[14], int Color) {
    ...
    who = Color;
	hand = 0;
	for(int i=0;i<32;i++)DeadTime[i]=150;
}
...
void BOARD::RoundCount(){
	hand++;
}

void BOARD::DeadRecord(){
	int alive_chess[14];
	for(int i=0;i<14;i++)alive_chess[i]=cnt[i];
	for(int i=0;i<32;i++)if(fin[i]<14)alive_chess[fin[i]]++;
	for(int i=0;i<7;i++){
		if(i==0){
			if(alive_chess[i]==0&&DeadTime[0]==150)
				DeadTime[0]=hand;
		}
		else if(i==6){
			switch(alive_chess[i]){
				case 0:
					if(DeadTime[11]==150)DeadTime[11]=hand;
					break;
				case 1:
					if(DeadTime[12]==150)DeadTime[12]=hand;
					break;			
				case 2:
					if(DeadTime[13]==150)DeadTime[13]=hand;
					break;				
				case 3:
					if(DeadTime[14]==150)DeadTime[14]=hand;
					break;				
				case 4:
					if(DeadTime[15]==150)DeadTime[15]=hand;
					break;					
			}
		}
		else{
			switch(alive_chess[i]){
				case 0:
					if(DeadTime[2*i-1]==150)DeadTime[2*i-1]=hand;
					break;
				case 1:
					if(DeadTime[2*i]==150)DeadTime[2*i]=hand;
					break;							
			}	
		}	
	}
	for(int i=7;i<14;i++){
		if(i==7){
			if(alive_chess[i]==0&&DeadTime[16]==150)
				DeadTime[16]=hand;
		}
		else if(i==13){
			switch(alive_chess[i]){
				case 0:
					if(DeadTime[27]==150)DeadTime[27]=hand;
					break;
				case 1:
					if(DeadTime[28]==150)DeadTime[28]=hand;
					break;			
				case 2:
					if(DeadTime[29]==150)DeadTime[29]=hand;
					break;				
				case 3:
					if(DeadTime[30]==150)DeadTime[30]=hand;
					break;				
				case 4:
					if(DeadTime[31]==150)DeadTime[31]=hand;
					break;					
			}
		}
		else{
			switch(alive_chess[i]){
				case 0:
					...
					break;
				case 1:
					if(DeadTime[2*i+2]==150)DeadTime[2*i+2]=hand;
					break;							
			}	
		}				
	}	
