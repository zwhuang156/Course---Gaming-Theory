...
SCORE Eval(const BOARD &B) {
	...
	for(POS p=0;p<32;p++){
		...
		if(c!=-1){
			switch(GetLevel(B.fin[p])){
				case 0: chess_power=200000;break;
				case 1: chess_power=100000;break;
				case 2: chess_power=33000;break;
				case 3: chess_power=10000;break;
				case 4: chess_power=3000;break;
				case 5: chess_power=30000;break;
				case 6:	chess_power=1400;break;
			}
			total_power[c]=total_power[c] + chess_power;		
		}
	}
	//用B.DeadTime去算個分數加到審局
	int kill_score=0;	//越小越好
	int multiple=6;
	if(B.who==0){
		for(int i=16;i<32;i++){
			if(i==16)kill_score=kill_score + 7*B.DeadTime[i];
			else if(i>26)kill_score+=B.DeadTime[i];
			else{			
				kill_score=kill_score + multiple*B.DeadTime[i];
				if(i%2==0)multiple--;
			}	
		}
	}
		...
		for(int i=0;i<16;i++){
			if(i==0)kill_score=kill_score + 7*B.DeadTime[i];
			else if(i>10)kill_score+=B.DeadTime[i];
			else{			
				kill_score=kill_score + multiple*B.DeadTime[i];
				if(i%2==0)multiple--;
			}	
	...
	return total_power[B.who]-total_power[B.who^1]-kill_score+distance(B);
}
...
//std::cout << "depth: "<<dep<<"\n";
	if(B.MoveGen(lst)==0){
		...
		for(int i=0;i<lst.num;i++){
			Q.Move(lst.mov[i]);
			if(Eval(Q)>temp_score)temp_score=Eval(Q);
		}
//for(int i=0;i<32;i++)std::cout<<"i: "<<	Q.DeadTime[i]<<"\n";	
//std::cout<<"num=0: "<<	-temp_score<<"\n";	
		return -temp_score;
	}
	if(cut==0||TimesUp()){
		//寧靜搜索
...
MOV Play(const BOARD &B) {
	...
	bool do_hash=0,mid_board=0,end_board=0;
	int dark = 0;
	for(int i=0;i<14;i++)dark+=B.cnt[i];
	if(dark<20){do_hash=1;mid_board=1;}
	if(dark<5){end_board=1;}
	
	// 若搜出來的結果會>現在 就用搜出來的走法
	MOVLST list;
	int useless=B.MoveGen(list);
	if(list.num!=0){
		BestMove=list.mov[rand()%list.num];//先隨便記錄一步 避免沒棋可翻又找不到更好的走步時會crash
		SCORE BestScore;
		if(mid_board!=1)
		{BestScore=NegaScout(B,0,8,-INF,INF,do_hash,do_hash,0);}
		else if(mid_board==1&&end_board!=1)
		{BestScore=NegaScout(B,0,14,-INF,INF,do_hash,do_hash,0);}	
		else
		{BestScore=NegaScout(B,0,22,-INF,INF,do_hash,do_hash,0);}
		
std::cout<<BestScore<<" "<<Eval(B)<<"\n";
		if(BestScore>Eval(B))return BestMove;
	}
	// 否則翻棋 但已經沒地方翻了的話
...
//system("pause");
	...
	safe_rate[33]=-1;
	for(p=0;p<32;p++){
		if(safe_rate[p]!=1&&safe_rate[p]>safe_rate[tmp_p]&&safe_rate[p]>=0.7)tmp_p=p;
	}
	p=tmp_p;
...
int distance(const BOARD &B){
	int distance;
	int dis[2] = {10000,10000};
	for (POS p = 0; p < 32; p++){
		const CLR c = GetColor(B.fin[p]);	
		for(POS d;d<32;d++){
			if(c!=-1&&GetColor(B.fin[d])!=-1&&GetColor(B.fin[d])!=c){
				distance=manhattan(p,d);
				switch(GetLevel(B.fin[p])){
					case 0:
						if(GetLevel(B.fin[d])==6)
							dis[c]=dis[c]-distance*50;
						else if(GetLevel(B.fin[d])==0)
							dis[B.who]=dis[B.who]+1000;
						else
							dis[c]=dis[c]-10*distance*(GetLevel(B.fin[d])-GetLevel(B.fin[p]));
						break;
					case 1:
					case 2:
					case 3:
					case 4:
						if(GetLevel(B.fin[p])<GetLevel(B.fin[d]))
							dis[c]=dis[c]-distance*(GetLevel(B.fin[d])-GetLevel(B.fin[p]));
						else if(GetLevel(B.fin[p])>GetLevel(B.fin[d]))
							dis[c]=dis[c]+distance*(GetLevel(B.fin[p])-GetLevel(B.fin[d]));
						else
							if(distance==1)
							dis[B.who]=dis[B.who]+120/GetLevel(B.fin[p]);
						break;
					case 5:break;
					case 6:
						if(GetLevel(B.fin[d])==0)
							dis[c]=dis[c]-distance*1000;
						else
							dis[c]=dis[c]-distance;
						break;
				}
			}
		}	
	}
	return dis[B.who] - dis[B.who ^ 1];
}
